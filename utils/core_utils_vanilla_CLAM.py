...
from models.model_vanilla_CLAM import CLAM_MB, CLAM_SB
...
def train(datasets, cur, args):
    ...
    print('\nInit optimizer ...', end=' ')
    optimizer = get_optim(model, args, lr=args.lr)
    print('Done!')
    ...
    for epoch in range(args.max_epochs):
        ...
        if args.model_type in ['clam_sb', 'clam_mb'] and not args.no_inst_cluster:     
            ...
            epoch_loss_total, train_loss=train_loop_clam(epoch, model, train_loader, optimizer, args.n_classes, args.bag_weight, writer, loss_fn)
            plot_losses_total.append(epoch_loss_total)
            plot_losses_train.append(train_loss)
            stop,epoch_val_loss, val_class_loss = validate_clam(cur, epoch, model, val_loader, args.n_classes,early_stopping, writer, loss_fn, args.results_dir)
    ...
    plt.ylabel('Loss')
    plt.legend(loc='best')
    plt.savefig(os.path.join(args.results_dir,"total_loss_for_{}_checkpoint.png".format(cur)))
    plt.clf()
    ...
    plt.ylabel('Loss')
    plt.legend(loc='best')
    plt.savefig(os.path.join(args.results_dir,"classification_loss_for_{}_checkpoint.png".format(cur)))
    plt.clf()
...
def train_loop_clam(epoch, model, loader, optimizer, n_classes, bag_weight, writer = None, loss_fn = None):
    ...
    for batch_idx, (data, label, coordinates, slide_id, _) in enumerate(loader): 
    ...
    return epoch_total_loss, train_loss
...
def validate_clam(cur, epoch, model, loader, n_classes, early_stopping = None, writer = None, loss_fn = None, results_dir = None):
    ...
    with torch.no_grad():
        for batch_idx, (data, label, _, _,_) in enumerate(loader):
            data, label = data.to(device), label.to(device)      
            ...
            error = calculate_error(Y_hat, label)
            val_error += error
            epoch_val_loss += 0.7*val_loss_value + 0.3* instance_loss_value
    ...
    if early_stopping:
        ...
        if early_stopping.early_stop:
            ...
            return True, epoch_val_loss, val_loss

    return False, epoch_val_loss, val_loss
