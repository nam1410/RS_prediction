...
from datasets.dataset_generic import save_splits #refer CLAM repository
from models.model_feedback_CLAM import CLAM_MB, CLAM_SB
...
def objective(trial, datasets, cur, args):
    ...
    """
    l2_weight = trial.suggest_categorical('l2_weight', [0.5, 5, 50, 500])
    lr = trial.suggest_categorical('lr', [2e-6,2e-5,2e-4,2e-3])
    ...
    for epoch in range(args.max_epochs):
        ...
        if args.model_type in ['clam_sb', 'clam_mb'] and not args.no_inst_cluster:     
            print("train_loop_clam")
            print("Learning rate", lr, "l2_weight", l2_weight)
            epoch_loss_total, train_loss, l2_loss_value = train_loop_clam(epoch, model, l2_weight, lr, train_loader, optimizer, args.n_classes, args.bag_weight, writer, loss_fn)
            plot_losses_total.append(epoch_loss_total)
            plot_losses_train.append(train_loss)
            plot_losses_feed.append(l2_loss_value)
            stop, epoch_val_loss, val_class_loss, l2_val_loss = validate_clam(cur, epoch, model,l2_weight, lr, val_loader, args.n_classes, early_stopping, writer, loss_fn, args.results_dir)
...
def train(datasets, cur, args):
    ...
    for l2_weight in [0.5, 5, 50, 500]:
        for lr in [2e-6,2e-5,2e-4,2e-3]:
            study.enqueue_trial({'l2_weight': l2_weight,'lr': lr})
    study.optimize(lambda trial: objective(trial, datasets, cur, args), n_trials=16)
...
def train_loop_clam(epoch, model,l2_weight,lr, loader, optimizer, n_classes, bag_weight, writer = None, loss_fn = None):
    ...
    previous_batch_id = 0
    current_batch_id = 0
    for batch_idx, (data, label, coordinates, slide_id, mn_M) in enumerate(loader): 
        if mn_M != None:
        	data, label, mn_M = data.to(device), label.to(device), mn_M.to(device)
        else:
        	data, label = data.to(device), label.to(device)
        logits, Y_prob, Y_hat, A_raw, instance_dict,  A_score, coeff, mn_new = model(data, mn_M, label=label, instance_eval=True)        
        ...
        l2_loss = F.mse_loss(A_score[label].unsqueeze(0),mn_new.unsqueeze(0))
        l2_val = l2_loss.item()
        total_loss =  loss + (1-bag_weight) * instance_loss + l2_weight * l2_loss
        ...
        if (batch_idx + 1) % 20 == 0:
            print('batch {}, loss: {:.4f}, instance_loss: {:.4f}, l2_loss: {:4f}, weighted_loss: {:.4f}, '.format(batch_idx, loss_value, instance_loss_value, l2_val, total_loss.item()) + 
                'label: {}, bag_size: {}, learning_rate: {}, l2_weight: {}'.format(label.item(), data.size(0), lr, l2_weight))
        error = calculate_error(Y_hat, label)
        train_error += error
        
        running_loss_total += total_loss.item()
        #a = list(model.parameters())[0].clone() #check computation graph
        total_loss.backward()
        #b = list(model.parameters())[0].clone() #check computation graph
        optimizer.step()
        #c = list(model.parameters())[0].clone() #check computation graph
        #print("check if model parameters are equal before update",torch.equal(a.data, b.data)) #check computation graph
        #print("check if model parameters are equal after update",torch.equal(a.data, c.data)) #check computation graph
        #print(torch.equal(b.data, c.data)) #check computation graph
        optimizer.zero_grad()
...
def validate_clam(cur, epoch, model, l2_weight, lr, loader, n_classes, early_stopping = None, writer = None, loss_fn = None, results_dir = None):
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    acc_logger = Accuracy_Logger(n_classes=n_classes)
    inst_logger = Accuracy_Logger(n_classes=n_classes)
    ...
    with torch.no_grad():
        for batch_idx, (data, label, _, _,mn_M) in enumerate(loader):
            if mn_M != None:
            	...
            	data, label = data.to(device), label.to(device)   
            logits, Y_prob, Y_hat, _, instance_dict, A_score, coeff, mn_new = model(data, mn_M, label=label, instance_eval=True)
            acc_logger.log(Y_hat, label)
            ...
            error = calculate_error(Y_hat, label)
            val_error += error
            #total_loss =  loss + 0.3 * instance_loss + 1 *l2_loss
            total_loss =  loss + 0.3 * instance_loss + l2_weight *l2_loss
...
def summary(model, loader, n_classes):
    ...
    for batch_idx, (data, label, _, _,mn_M) in enumerate(loader):
        ...
        with torch.no_grad():
            logits, Y_prob, Y_hat, _, _,_,_,_ = model(data, mn_M)
